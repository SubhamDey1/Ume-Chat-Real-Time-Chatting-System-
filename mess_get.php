<?php
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if(isset($_POST['c_userid']) && isset($_POST['dark_mode'])) {
            $info['error'] = 1;
            require_once "conn.php";
            require "time_ago.php";
            $conn = connected() or die("Connection Failed");
            $current_user_id = mysqli_real_escape_string($conn,trim($_POST['c_userid']));
            ...
            if(mysqli_num_rows($mysqli_result_check) == 1) {
                ...
                if(mysqli_num_rows($result)>0) {
                    ...
                    while($arr = mysqli_fetch_assoc($result)) {
                        ...
                        if($login_user_id == $sender_id && $current_user_id == $receiver_id) {
                            ...
                            if($arr['type'] == "message") {
                                $info['message__data'] .= "<div class='chat_message_and_time chat_sent chat_sent_light_mode_style $dark_mode_class_chat_send'>
                                ...
                                $info['message__data'] .= "<div class='chat_message_and_time img_video_outer_ele_ chat_sent chat_sent_light_mode_style $dark_mode_class_chat_send'>
                ...
                if($current_user_message_data_id != "") {
                    $current_user_message_data_id = substr($current_user_message_data_id, 0, (strlen($current_user_message_data_id) - 1));
                }
